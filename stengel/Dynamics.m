...
%	Copyright 2008 by ROBERT F. STENGEL.  All rights reserved.
	...
	global m Ixx Iyy Izz Ixz S b c flightsys
...
%	Air-Relative Velocity Vector
    ...
    flightsys.alpha = alpha;
    flightsys.beta = beta; 
    flightsys.alphar=alphar;
    flightsys.betar=betar;
    flightsys.b2e = HEB;
    flightsys.V = V;

%	Force and Moment Coefficients; Thrust	
	[CD,CL,CY,Cl,Cm,Cn,Thrust]	=	AeroModel(x,uTotal,Mach,alphar,betar,V);
    % these all come from data tables, how do we get a data table for our
    % plane?
	qbarS	=	qbar * S;
    flightsys.qbarS=qbarS;
    flightsys.thrust = Thrust;
    flightsys.mass = m;
    flightsys.chord = c;
...
%	State Accelerations
	...
	Xb =	(CX * qbarS + Thrust*sin(betat)) / m; %is this only for when thrust in x-dir?
	Yb =	CY * qbarS / m;
	Zb =	(CZ * qbarS+ Thrust*cos(betat)) / m; %when rotors at beta=0, add thrust?
	Lb =	Cl * qbarS * b;
	Mb =	Cm * qbarS * c;
	Nb =	Cn * qbarS * b;
...
%	Dynamic Equations
	...
	xdot	=	[xd1;xd2;xd3;xd4;xd5;xd6;xd7;xd8;xd9;xd10;xd11;xd12;0;0;0;0;0;0;0];